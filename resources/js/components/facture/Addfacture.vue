<template>
  <div>
    ...
    <form @submit.prevent="addFacture">
      ...
      <table class="inventory">
        ...
        <tbody>
          <tr>
            ...
            <!-- Sélection du Nom avec possibilité de changer le dépôt -->
            <td>
              <a class="cut">-</a>
           ...
           <!--  <td>{{ facture.Nom }}</td>-->
            ...
            <!-- Prix unitaire, Total HT, TVA et Total -->
            <td><input id="Reference-1-qte" v-on:change="onChangeqte($event)" type="number" min="1" v-model="selectedQuantity" /></td>
      ...
      <!-- Tableau pour le total, la remise et le total TTC -->
      <table class="balance">
        <tr>
...
const addFacture = async () => {
  try {
    console.log(facture);
    await axios.post("http://localhost:8000/api/factures/", facture.value);
    router.push({ name: 'Viewfacture' });
  } catch (err) {
...
const calculateTotalTTC = computed(() => {
  ...
  const remisePercentage = remise.value;
facture.value.TotalTTC=total - ((total * remise.value)/100);
  return total - ((total * remise.value)/100);
});
